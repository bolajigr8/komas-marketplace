...
//   // Load default Rivers State station when component mounts
//   useEffect(() => {
//     const loadDefaultStation = async () => {
//       if (!defaultStationLoaded) {
//         setLoading(true)
//         try {
//           console.log('Loading default Rivers State pickup stations...')
//           const response = await fetchPickupStations({ region: 'Rivers' })
//           const stations = response?.data || []

//           console.log('Default Rivers State stations found:', stations)
...
//           }
//         } catch (err) {
//           console.error('Error loading default Rivers State stations:', err)
//         } finally {
//           setLoading(false)
...
//       // Use 'region' parameter for API call instead of 'state'
//       const response = await fetchPickupStations({ region: formattedState })
//       const stations = response?.data || []
...
//                     placeholder='e.g. Rivers, Lagos, Abuja, Kano'
//                     className='mb-2'
//                   />
//                   <Button
//                     color='primary'
...
const PickupStations: React.FC<PickupStationProps> = ({
    ...
    const loadDefaultStation = async () => {
      if (!defaultStationLoaded) {
        ...
        try {
          ...
          if (stations.length > 0) {
            setPickupStations(stations)
            setSelectedStation(stations[0]) // Auto-select first station for Rivers
            setState('Rivers')
            setHasSearched(true)
            console.log(
              'Default Rivers State station auto-selected:',
              stations[0]
            )
            console.log('Total Rivers State stations loaded:', stations.length)
          } else {
            console.log('No default stations found for Rivers State')
          }
        } catch (err) {
          console.error('Error loading default Rivers State stations:', err)
        } finally {
  ...
  const handleStateSubmit = async () => {
    ...
    setError(null)
    setHasSearched(true)
    setSelectedStation(null) // Clear previous selection when searching
    ...
    try {
      ...
      // Use 'region' parameter for API call
      const response = await fetchPickupStations({ region: formattedState })
      const stations = response?.data || []
      ...
      // Don't auto-select - let user choose from all available stations
      if (stations.length === 0) {
        console.log('No stations found for:', formattedState)
      }
    } catch (err) {
      console.error('Error fetching pickup stations:', err)
      setError('Failed to fetch pickup stations. Please try again.')
  ...
  return (
    <Modal
      ...
      backdrop='blur'
      placement='center'
      className='w-[75%] h-[80%] overflow-y-auto max-w-none'
    >
      <ModalContent>
        {(onClose) => (
          <>
              ...
              <div className='flex flex-col md:flex-row gap-6 h-full'>
                ...
                <div className='flex-1 overflow-auto h-full border p-4 rounded'>
                  {pickupStations.length > 0 && (
                    <>
                      <p className='text-sm font-medium mb-3 text-gray-700'>
                        Available Stations ({pickupStations.length}):
                        {selectedStation && (

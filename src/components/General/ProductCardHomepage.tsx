...
const ProductCardHomepage = memo(
  ({
    ...
    // Optimized sizes attribute for homepage grid layout
    const getSizesAttribute = (isSlider = false) => {
      return '(max-width: 375px) 95vw, (max-width: 640px) 85vw, (max-width: 768px) 45vw, (max-width: 1024px) 30vw, (max-width: 1280px) 25vw, 20vw'
    }
    ...
    return (
      <Card
        key={product._id}
        className={`group hover:shadow-xl hover:shadow-black/5 transition-all duration-300 hover:-translate-y-1 h-full overflow-hidden border-0 shadow-sm bg-white w-full max-w-sm mx-auto ${className}`}
      >
        <div className='flex flex-col justify-between h-full w-full'>
          {/* Image Container with same styling as ProductCard */}
          <div className='relative overflow-hidden bg-white aspect-[4/3] w-full min-h-[220px] max-h-[400px] sm:min-h-[280px] sm:max-h-[350px] rounded-t-lg'>
            {/* Discount badge */}
          ...
          {/* Content Container with same styling as ProductCard */}
          <div className='p-4 sm:p-5 flex flex-col bg-white flex-grow'>
            <Link
              href={`/product/${product._id}`}

...
const ProductCardSlider = memo(
  ({
    ...
    // Memoized cart item check - Fixed selector usage
    const cartProducts = useAppSelector((state) => state.cart.products)
    const cartItem = useMemo(() => {
      if (hasVariants) {
        return cartProducts.find(
    ...
    return (
      <>
        <Card
          className={`group hover:shadow-lg transition-shadow duration-300 h-full overflow-hidden ${className}`}
        >
          <div className='flex flex-col h-full'>
            {/* Image Container */}
            <div
              ref={imageContainerRef}
              className='relative overflow-hidden bg-gray-50 aspect-[4/5] min-h-[280px] max-h-[400px]'
            >
              {/* Discount badge */}
              {product.discount && product.discount > 0 && (
            ...
            <div className='p-4 flex flex-col flex-grow bg-white'>
              ...
              <div className='flex items-center justify-between mt-auto'>
                <div className='flex-1 mr-2'>
                  <p className='font-bold text-lg text-[#3bb77e]'>
                    ₦
                    {(
                      product.price *
                      (1 - (product.discount || 0) / 100)
                ...
                {showCartBtn && (
                  <div className='flex-shrink-0'>
                    {cartItem && !hasVariants ? (
                      ...
                      <Button
                        size='icon'
                        className='bg-[#3bb77e] hover:bg-[#2ea56c] text-white w-10 h-10 rounded-full'
                        onClick={handleAddToCart}
                        disabled={!isSync}
                        aria-label='Add to cart'
                      >

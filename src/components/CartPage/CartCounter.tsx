...
type PropsType = {
  ...
  showQuickEdit?: boolean
  className?: string
  path?: 'cart' | 'product' | 'other' // New prop to determine context
}
type QuantitySize = 'sm' | 'md' | 'lg'
...
const CartCounter = ({
  ...
  // Handle delete functionality - removes entire item from cart
  ...
  return (
    <div
      className={`flex items-center ${
        path === 'cart' ? 'gap-2' : 'gap-1'
      } ${className}`}
    >
      {/* Quantity counter */}
      <div
        className={`flex items-center gap-1 bg-[#3bb77e]/10 rounded-full ${wrapper} ${
          isLoading ? 'opacity-50 pointer-events-none' : ''
        }`}
      >
        <button
      ...
      {/* Delete button - only shows when path is 'cart' */}
      {path === 'cart' && (
        <button
          onClick={handleDelete}
          disabled={isLoading}
          className={`text-red-500 disabled:opacity-40 hover:bg-red-50 rounded-full transition-colors ${button}`}

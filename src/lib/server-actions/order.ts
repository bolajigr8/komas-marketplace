  ...
  try {
    ...
    if (!response.ok) {
      ...
      try {
        ...
        logOrderDetails('ERROR_PARSE_FAILED', { parseError: 'Failed to parse error response' })
    ...
    try {
      ...
      logOrderDetails('SUCCESS_PARSE_ERROR', { parseError: 'Failed to parse success response' })
...
export const checkoutShopmateOrder = async (orderData: ShopmateOrderData) => {
  try {
    ...
    // Get response text
    const responseText = await response.text()
    logOrderDetails('SHOPMATE_RAW_RESPONSE_LENGTH', { length: responseText.length })
    ...
    if (!response.ok) {
      ...
      try {
        ...
        logOrderDetails('SHOPMATE_ERROR_PARSE_FAILED', { parseError: 'Failed to parse error response' })
    ...
    try {
      ...
      logOrderDetails('SHOPMATE_SUCCESS_PARSE_ERROR', { parseError: 'Failed to parse success response' })
    ...
    const errorMessage = error instanceof Error ? error.message : 'Shopmate order creation failed'

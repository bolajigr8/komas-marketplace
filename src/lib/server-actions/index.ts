...
// custom fetch is causing issues so for the fetchwith auth i want to use normal fetch

// For cart-specific authenticated requests
export const fetchCartWithAuth = async (
  input: RequestInfo | URL,
  init?: RequestInit
): Promise<Response> => {
  try {
    ...
    // Log the cart request details (without sensitive data)
    console.log('Making cart authenticated request:', {
      url: fullUrl,
      method: init?.method || 'GET',
      hasAuth: headers.Authorization?.startsWith('Bearer '),
    })
    ...
    console.log('Cart request completed:', {
      status: response.status,
      statusText: response.statusText,
      url: fullUrl,
    })
    ...
    console.error('fetchCartWithAuth error:', error)
    throw error
  }
}

...
const getItemPrice = (item: CartItem): number => {
  ...
  // Check if variant exists and is not null
  if (item.variant && item.variant !== null) {
    // Use variant price if available, otherwise fallback to product price
    const variantPriceRaw = item.variant.price
    const productPriceRaw = item.product.priceWithMarkup ?? item.product.price

    const variantPrice =
      typeof variantPriceRaw === 'string'
    ...
    // No variant, use regular product price logic
    price = item.product.priceWithMarkup ?? item.product.price ?? 0
  }
                  ...
                  <div className='flex-1'>
                    ...
                    {item.variant && item.variant !== null && (
                      <p className='text-xs text-blue-600'>
                        Variant: {item.variant.name || 'Selected variant'}
                      </p>
                    )}
                  </div>
                  <p className='font-medium text-green-500'>
